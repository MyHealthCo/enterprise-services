...
resource "aws_route_table" "compute_native" {
  provider = aws.use2
  vpc_id   = aws_vpc.main.id

  tags = {
    Name = "Compute-Native-RouteTable"
  }
}
...
resource "aws_route_table_association" "compute_native_a" {
  provider       = aws.use2
  subnet_id      = aws_subnet.compute_native_a.id
  route_table_id = aws_route_table.compute_native.id
}

resource "aws_route_table_association" "compute_native_b" {
  provider       = aws.use2
  subnet_id      = aws_subnet.compute_native_b.id
  route_table_id = aws_route_table.compute_native.id
}

resource "aws_route_table_association" "compute_native_c" {
  provider       = aws.use2
