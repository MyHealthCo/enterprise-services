...
resource "aws_vpc_endpoint_policy" "eks" {
  provider        = aws.use2
  vpc_endpoint_id = aws_vpc_endpoint.eks.id

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid       = "AllowEKSAccessOnlyFromOrganization"
        Effect    = "Allow"
        Principal = "*"
        Action    = "eks:*"
        Resource  = "*"
...
resource "aws_vpc_endpoint_policy" "sts" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
        Sid       = "AllowSTSAccessOnlyFromOrganization"
        Effect    = "Allow"
        Principal = "*"
        Action    = "sts:*"
        Resource  = "*"
